...
def format_message_for_memory(message: AnyMessage) -> str:
    """格式化消息用于记忆存储，处理图片描述。
    ...
    # 检查是否有图片
    has_image = has_image_content(message.content)

    if not has_image:
        return text

    # 从 additional_kwargs 获取图片描述
    image_description = None
    if isinstance(message, HumanMessage):
        additional_kwargs = getattr(message, "additional_kwargs", None)
        if additional_kwargs:
            image_description = additional_kwargs.get("image_description")

    # 格式化输出
    if image_description:
        return f"[发送了图片：{image_description}] {text}"
    else:
        return f"[发送了图片] {text}"
