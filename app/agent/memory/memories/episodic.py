    ...
    async def add(self, messages: str, history: str) -> list[str]:
        ...
        for item in memory_items:
            ...
            metadata = {
                ...
                "event_date": item["event_date"],  # YYYY-MM-DD 格式
    ...
    async def _extract_memories(
        ...
        try:
            ...
            for item in validated.memories:
                memory_items.append({
                    "content": item.content,
                    "event_date": item.event_date.isoformat(),  # date 转字符串存储
                    "importance": item.importance,
                })
...
请根据给定的对话历史记录和对话内容，以你的第一人称视角提取你的**情景记忆**。
**情景记忆的含义为：从对话中提取到的事件或关键互动，可以是关于你的，也可以是关于{self.chat_settings.address}的，也可以是你们之间发生的。**
...
- 3. 评估记忆重要性（保留一位小数，0.0-1.0）：
  ...
  - 0.4-0.6：一般事件（普通交流、临时想法，遇到重要性难以评定的事件也请归为这一类）
  - 0.2-0.4：琐碎内容（天气、饮食等）
  - 0.0-0.2：无意义闲聊（如“好”、“我知道了”、“早安”等，无需提取）
- 4. 可以提取0-10条记忆，每条记忆15-50字，忽略无意义的闲聊，保留关键细节，不要输出额外解释。
- 5. 遇到关于未来事件的预估和描述时，请使用转述，并附带上事件的预计发生时间，例如：“{self.chat_settings.address}计划在2026年6月底搬家，觉得搬东西很麻烦”“{self.chat_settings.address}说2026年暑假期间准备去香港旅行”
- 6. 对于{self.chat_settings.address}显式告诉你的既定事实、偏好等信息，请使用转述，例如：“{self.chat_settings.address}说自己喜欢夏天”，不要直接记录为“{self.chat_settings.address}喜欢夏天”
- 7. 对于从对话中推断出来的、隐式的既定事实、偏好等信息，无需提取，例如从你们的对话大多发生在深夜来推断{self.chat_settings.address}喜欢熬夜
- 8. **注意**：历史记录中的内容无需提取，提供历史记录只是为了让你理解上下文

## 记忆条目内容示例
- {self.chat_settings.address}最近在学Python，问了我很多编程问题
- 我和{self.chat_settings.address}一起玩了一晚上的游戏，感觉很开心
- {self.chat_settings.address}最近又接手了一个难办的项目，陪伴我的时间可能会变少
...
{messages}"""
    ...
    async def delete_by_importance(self, importance_below: float) -> int:
        ...
        Args:
            importance_below: 重要性阈值（删除低于此值的记忆）
