...
class SemanticMemory:
    ...
    # 类级别的 spaCy 模型缓存（避免每个实例重复加载）
    _nlp = None
    _nlp_lock = threading.Lock()  # 防止多线程竞态
    ...
    async def add(self, messages: str, history: str) -> list[str]:
        ...
        for item in memory_items:
            ...
            # 2.2 查找冲突（仅对单值属性且有完整三元组的情况）
            # 注意：先存储新记忆，成功后再标记旧记忆过期，避免存储失败导致数据丢失
            conflicts = []
            if (item.get("subject") and item.get("relation") and item.get("obj")
            ...
            # 2.4 存储到 Chroma 和 Neo4j（失败时回滚 SQLite）
            try:
                # Chroma 存储
                metadata = {
                    "session_id": self.session_id,
    ...
    async def _is_duplicate(self, content: str, threshold: float = 0.95) -> bool:
        ...
        # 二次验证：检查 SQLite 中的 is_current 状态
    ...
    async def _mark_obsolete(self, old_id: str, new_id: str):
        ...
        # 2. Chroma 同步更新 metadata（过滤 None 值，布尔值转整数）
        try:
    ...
    async def _extract_memories(
        ...
        try:
            structured_llm = self.llm.with_structured_output(SEMANTIC_MEMORY_SCHEMA)
            ...
            for item in validated.memories:
                memory_items.append({
                    ...
                    "subject": item.subject,
                    "relation": item.relation,
                    "obj": item.object,  # Pydantic 模型仍用 object，转换时映射
...
{messages}"""
    ...
    async def _vector_search(self, query: str, top_k: int) -> list[MemoryItem]:
        ...
        for result in results:
            ...
            # 从 SQLite 获取完整记录（权威数据源）
            record = await self.sqlite_store.get(memory_id)
            if not record:
                continue

            # 二次验证：确保 SQLite 中的 is_current 状态正确
            # 防止 Chroma metadata 更新失败导致返回过期记忆
    ...
    def _extract_query_entities(self, query: str) -> list[str]:
        ...
        # 1. 固定实体（字符串匹配，100% 准确）
        # chat_settings.address: Agent 对用户的称呼（如"主人"）
        # 用户查询中出现这个词时，应该匹配"用户"
        if "用户" in query or self.chat_settings.address in query:
            entities.append("用户")

        # chat_settings.name: Agent 的名字（如"日和"）
        # "你" 或 Agent 名字出现时，应该匹配"我"（Agent 自身）
        ...
        if nlp is not None:
            try:
                doc = nlp(query)
                for ent in doc.ents:
                    # 只保留有意义的实体类型，过滤掉 DATE、TIME、CARDINAL 等
    ...
    def _combine_and_rank(
        ...
        Args:
            ...
            graph_results: 图检索结果（已转换为 MemoryItem）
            query: 查询文本

        Returns:
            排序后的记忆列表
        """
        # 用字典去重，key 为 memory_id
