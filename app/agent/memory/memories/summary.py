...
请生成{summary_type}（200字以内）："""
    ...
    def _format_diaries_with_gaps(
        ...
        while check_date > earliest_diary:  # 不包含 earliest_diary
        ...
        # 处理最后一个 gap（如果到 earliest_diary 的下一天还在缺失）
        if current_gap_start is not None:
        ...
        for gap_start, gap_end in gap_ranges:
            # gap_start > gap_end，gap_end 是缺失区间的后一天
            # 缺失区间是 [gap_end, gap_start]
    ...
    def _format_messages(self, messages: list[dict[str, Any]]) -> str:
        ...
        Args:
            messages: 聊天记录列表，timestamp 已由数据层格式化为本地时间
