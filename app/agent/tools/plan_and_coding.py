...
from app.agent.subagents.coding import run_coding_subagent
...
@tool
async def plan_and_coding(
    raw_items: Annotated[str, "编程任务的待办事项列表，每行一项，最多20项。"],
    command: Annotated[str, "对编程专家下的命令，如：用python画一个爱心"],
    context: ToolContext,
) -> str:
    """创建编程任务的待办事项列表，并命令编程专家agent进行编程。需要进行编程时使用该工具。请将任务的不同步骤精确地拆解成多个待办事项，任务代码量大时，可将其按不同文件模块拆分成多个待办事项。如任务是新建项目，必须要有检查/安装环境依赖、创建xxx.py文件并写入代码、运行测试等步骤；如任务是修复bug，必须要有定位问题、修改代码、验证测试等步骤，确保编程专家能按待办事项逐步完成任务。command中必须完整清晰地描述要完成的任务或要修复的bug。"""
