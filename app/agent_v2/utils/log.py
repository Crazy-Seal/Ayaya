...
# 解析到 app/agent_v2/tools/tools.log
...
async def log_tool_call_result(
    ...
    result: Any,
    *,
    logger_name: str = "app.agent_v2.tools",
) -> Any:
    """为 class-based 工具记录一次调用（参数 + 输出）。

    agent_v2 的工具是类方法，无法套用旧的函数装饰器，这里提供一个可直接调用的
...
def log_tool_call(logger_name: str = "app.agent_v2.tools"):
